...
assert.match(index, /manifest\.webmanifest\?v=7/);
...
assert.match(index, /provider-ux\.css\?v=7/);
assert.match(index, /site-update\.js\?v=7/);
assert.match(index, /provider\.js\?v=7/);
assert.match(index, /provider-schedule-desktop-reference\.css\?v=7[^>]+media="\(min-width: 761px\)"/);
assert.match(index, /class="provider-view mobile-more-overlay" data-provider-panel="more"/);
assert.match(worker, /provider-ux\.css\?v=7/);
assert.match(worker, /site-update\.js\?v=7/);
assert.match(worker, /provider\.js\?v=7/);
assert.match(worker, /manifest\.webmanifest\?v=7/);
assert.match(worker, /provider-feature-assets\.js\?v=7/);
assert.match(worker, /provider-schedule-desktop-reference\.css\?v=7/);
...
assert.match(read('provider-schedule-minimal.css'), /--schedule-active-color:var\(--theme-accent\)/);
assert.match(read('provider-schedule-minimal.css'), /background:var\(--schedule-active-color\)!important/);
assert.match(read('provider-schedule-minimal.css'), /v833: theme-led mobile summary and schedule hierarchy/);
assert.match(read('provider-schedule-desktop-reference.css'), /--pt-desktop-accent:#12935f/);
assert.match(provider, /mobileCenteredRange[\s\S]*rangeMode = mobileCenteredRange \? 'inertial' : 'extended'/);
assert.match(provider, /renderDateStrip\(\{ forceCenter:true \}\)/);
assert.match(provider, /scheduleCreateHintVisible[\s\S]*localStorage\.setItem\(key, 'dismissed'\)/);
...
assert.doesNotMatch(provider, /function dateStripSwipeStep/);
assert.match(provider, /touchIntent !== 'horizontal'/);
assert.match(provider, /centerDateStripSelection\(dateStrip, \{ instant:true \}\)/);
