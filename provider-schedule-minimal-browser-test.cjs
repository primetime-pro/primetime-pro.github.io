...
(async () => {
  ...
  try {
    ...
    for (const width of [390, 760]) {
      ...
      const stripBox = await page.locator('#dateStrip').boundingBox();
      await page.mouse.move(stripBox.x + stripBox.width * .72, stripBox.y + stripBox.height / 2);
      await page.mouse.down();
      await page.mouse.move(stripBox.x + stripBox.width * .28, stripBox.y + stripBox.height / 2, { steps:4 });
      await page.mouse.up();
      const pointerDrag = await page.evaluate(() => {
        ...
        const activeRect = active.getBoundingClientRect();
        return {
          active:active.dataset.bookingDate,
          picker:document.querySelector('#scheduleDatePicker').value,
          centerDelta:Math.abs((activeRect.left + activeRect.right - stripRect.left - stripRect.right) / 2)
        };
      });
      assert.equal(pointerDrag.active, pointerDrag.picker, `${width}px: мышиный drag рассинхронизировал дату`);
      assert.ok(pointerDrag.centerDelta <= 1, `${width}px: после мышиного drag выбранная дата ушла из центра (${pointerDrag.centerDelta}px)`);
    }
